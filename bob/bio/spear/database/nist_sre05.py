...
"""
  NIST SRE 2005 database implementation of bob.bio.db.Database interface.
  It is an extension of an SQL-based database interface, which directly talks to NIST SRE 2005 database, for
...
class NistSre05BioFile(AudioBioFile):
    def __init__(self, f):
        """
        Initializes this File object with an File equivalent from the underlying SQl-based interface for
        NIST SRE 2005 database.
        """
        super(NistSre05BioFile, self).__init__(client_id=f.client_id, path=f.path, file_id=f.id)
...
class NistSre05BioDatabase(BioDatabase):
    """
    Implements verification API for querying NIST SRE 2005 database.
    """

    def __init__(
            self,
            protocol='1conv4w-1conv4w',
            **kwargs
    ):
        # call base class constructors to open a session to the database
        super(NistSre05BioDatabase, self).__init__(
            name='nist_sre05',
            protocol=protocol,
            **kwargs)


        from bob.db.nist_sre05.query import Database as LowLevelDatabase
    ...
    def objects(self, groups=None, protocol=None, purposes=None, model_ids=None, **kwargs):
        ...
        return [NistSre05BioFile(f) for f in retval]
