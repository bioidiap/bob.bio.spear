...
"""
  NIST SRE 2010 database implementation of bob.bio.db.Database interface.
  It is an extension of an SQL-based database interface, which directly talks to NIST SRE 2010 database, for
...
class NistSre10BioFile(AudioBioFile):
    def __init__(self, f):
        """
        Initializes this File object with an File equivalent from the underlying SQl-based interface for
        NIST SRE 2010 database.
        """
#        super(NistSre10BioFile, self).__init__(client_id=f.client_id, path=f.path, file_id=f.id)
        super(NistSre10BioFile, self).__init__(client_id='M_ID_X', path=f.path, file_id=f.id)
...
class NistSre10BioDatabase(BioDatabase):
    """
    Implements verification API for querying NIST SRE 2010 database.
    """
    ...
    def __init__(
        ...
        super(NistSre10BioDatabase, self).__init__(
            name='nist_sre10',
            **kwargs)


        from bob.db.nist_sre10.query import Database as LowLevelDatabase
    ...
    def objects(self, groups=None, protocol=None, purposes=None, model_ids=None, **kwargs):
        ...
        return [NistSre10BioFile(f) for f in retval]
