...
"""
  NIST SRE 2006 database implementation of bob.bio.db.Database interface.
  It is an extension of an SQL-based database interface, which directly talks to NIST SRE 2006 database, for
...
class NistSre06BioFile(AudioBioFile):
    def __init__(self, f):
        """
        Initializes this File object with an File equivalent from the underlying SQl-based interface for
        NIST SRE 2006 database.
        """
        super(NistSre06BioFile, self).__init__(client_id=f.client_id, path=f.path, file_id=f.id)
...
class NistSre06BioDatabase(BioDatabase):
    """
    Implements verification API for querying NIST SRE 2006 database.
    """
    ...
    def __init__(
        ...
        super(NistSre06BioDatabase, self).__init__(
            name='nist_sre06',
            protocol=protocol,
            **kwargs)


        from bob.db.nist_sre06.query import Database as LowLevelDatabase
    ...
    def objects(self, groups=None, protocol=None, purposes=None, model_ids=None, **kwargs):
        ...
        return [NistSre06BioFile(f) for f in retval]
